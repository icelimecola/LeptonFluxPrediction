...
FLUX_PATH = Path('Data/flux/positron_flux_allbin.npy')
ERR_PATH = Path('Data/flux/positron_flux_abs_error_allbin.npy')
ENERGY_PATH = Path('Data/flux/positron_energy_edges.npy')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Draw positron flux vs solar-parameter lag diagnostic figures.'
    )
...
def save_zscore_overlay(dates, flux_smooth, sun_smooth, plot_bins, bin_labels, train_end, val_end):
    ...
    fig.suptitle('Z-score positron flux and solar parameters', fontsize=16)
...
def save_faceted_timeseries(dates, flux, err, sun, flux_smooth, sun_smooth,
    ...
    for bi, label in zip(plot_bins, bin_labels):
        ...
        fig.suptitle('Positron flux and solar parameters', fontsize=16)

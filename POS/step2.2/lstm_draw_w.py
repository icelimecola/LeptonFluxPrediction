...
FLUX_PATH = 'Data/flux/positron_flux_allbin.npy'
ERROR_PATH = 'Data/flux/positron_flux_abs_error_allbin.npy'


def load_energy_edges():
    path = Path('Data/flux/positron_energy_edges.npy')
    if path.exists():
        print('positron energy edges path : ', path)
        return np.load(path)
    raise FileNotFoundError('No positron_energy_edges.npy found in step2.2/Data/flux')
...
positron_daily = np.load(FLUX_PATH)
positron_error_daily = np.load(ERROR_PATH)

print('sun daily all latest : ', sun_daily.shape)
print('positron flux daily : ',  positron_daily.shape)
print('positron error daily : ',  positron_error_daily.shape)
print('positron flux path : ', FLUX_PATH)
print('positron error path : ', ERROR_PATH)

# 正电子前补 365 天零
pad_days  = 365
positron_daily = np.concatenate([np.zeros([pad_days, positron_daily.shape[1]]), positron_daily])
positron_error_daily = np.concatenate([np.zeros([pad_days, positron_error_daily.shape[1]]), positron_error_daily])
...
y_train = positron_daily[0:train_end, :]
err_train_origin = positron_error_daily[0:train_end, :]
X_val   = X_all_raw[train_end-look_back:val_end, :]
y_val   = positron_daily[train_end-look_back:val_end, :]
err_val_origin = positron_error_daily[train_end-look_back:val_end, :]
X_test  = X_all_raw[val_end-look_back:test_end, :]
y_test  = positron_daily[val_end-look_back:test_end, :]
err_test_origin = positron_error_daily[val_end-look_back:test_end, :]
...
def write_prediction_html(output_path, idx, plot_bins, bin_labels,
    ...
    write_plotly_panels(
        output_path, 'Sun-Only Weighted Positron Flux Prediction', panels,
...
def write_error_html(output_path, idx, plot_bins, bin_labels, train_error, val_error, test_error):
    ...
    write_plotly_panels(
        output_path, 'Sun-Only Weighted Positron Flux Relative Error', panels,
...
for m in model_list:
    ...
    plt.savefig('./Figure/lstmdraww/positron_prediction_' + m + '.pdf', bbox_inches='tight')
    plt.close()
    write_prediction_html(
        './Figure/lstmdraww/positron_prediction_' + m + '.html',
    ...
    fig.supxlabel('Year', y=0.03, fontsize=14)
    plt.savefig('./Figure/lstmdraww/positron_error_' + m + '.pdf', bbox_inches='tight')
    plt.close()
    write_error_html(
        './Figure/lstmdraww/positron_error_' + m + '.html',

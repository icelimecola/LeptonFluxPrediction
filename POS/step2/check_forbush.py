...
# =====================================================
# 1. 读取 positron flux ROOT 文件
# =====================================================
# ROOT 文件沿用 dataproc_fluxpos.py 的输入：
...
def read_flux_from_root(root_file):
    ...
    # 与 dataproc_fluxpos.py 保持一致：跳过 [0.8, 1.0] 和 [41.9, 45.1] GeV。
    keep_bins = np.arange(1, h2.axis(1).member("fNbins") - 1)
    flux_2d = flux_2d[:, keep_bins]
    err_2d = err_2d[:, keep_bins]
    y_edges = y_edges[keep_bins[0] : keep_bins[-1] + 2]

    # positron flux 不应为 0。ROOT 里的 0 按 missing data 处理，画图时不连假点。
    zero_mask = flux_2d == 0
...
def draw_forbush(center_date, root_file, outdir, targets):
    ...
    pdf_file = outdir / f"positron_forbush_decrease_{date_tag}.pdf"
    html_file = outdir / f"positron_forbush_decrease_{date_tag}.html"

    plt.subplots_adjust(left=0.08, right=0.97, top=0.93, bottom=0.10, wspace=0.08, hspace=0.12)
    fig.suptitle(f"Positron Flux around {center.strftime('%Y-%m-%d')} (13 days)", fontsize=15)
    ...
    write_plotly_panels(
        html_file,
        f"Positron Flux around {center.strftime('%Y-%m-%d')} (13 days)",
        panels,
        columns=2,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Draw 13-day positron flux with errors around a given Forbush decrease date."
    )
    parser.add_argument("date", help="Center date, e.g. 2017-09-08")
    parser.add_argument(
        "--root-file",
        default=here.parent.parent / "rawdata" / "flux" / "posflux.root",
        type=Path,
        help="Input ROOT file. Default: ../../rawdata/flux/posflux.root",
    )
    parser.add_argument(
        "--outdir",

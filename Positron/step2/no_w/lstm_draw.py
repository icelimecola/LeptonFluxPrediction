...
def load_energy_edges():
    candidates = [
        Path('Data/flux/positron_energy_edges.npy'),
        Path('../step1/Data/flux/positron_energy_edges.npy'),
    ]
    for path in candidates:
        if path.exists():
            print('positron energy edges path : ', path)
            return np.load(path)
    raise FileNotFoundError('No positron_energy_edges.npy found in step2/Data/flux or step1/Data/flux')
...
positron_daily = np.load('Data/flux/positron_flux_allbin.npy')

print('sun daily all latest : ', sun_daily.shape)
print('positron flux daily : ',  positron_daily.shape)

# 正电子前补 365 天零
pad_days  = 365
positron_daily = np.concatenate([np.zeros([pad_days, positron_daily.shape[1]]), positron_daily])
...
number = positron_daily.shape[0]
bins = positron_daily.shape[1]
...
# 只组合观测数据
Series = np.concatenate([sun_daily[sun_offset:sun_offset+number], positron_daily], axis=1)
...
# 5. 填充未来待预测数据
# =====================================================
# 未来天数 = sun 总长 - (sun 偏移 + positron 天数)
...
def write_prediction_html(output_path, idx, plot_bins, bin_labels,
    ...
    for j, bi in enumerate(plot_bins):
        title = bin_labels[j]
        raw_traces = [
            ('observation', idx[look_back:train_end], train_true[look_back:, bi], 'green', 'lines'),
            ('observation', idx[train_end:val_end], val_true[:, bi], 'green', 'lines'),
            ('observation', idx[val_end:test_end], test_true[:, bi], 'green', 'lines'),
            ('training prediction', idx[look_back:train_end], train_pred[:, bi], 'blue', 'lines'),
            ('validation prediction', idx[train_end:val_end], val_pred[:, bi], 'goldenrod', 'lines'),
            ('test prediction', idx[val_end:test_end], test_pred[:, bi], 'magenta', 'lines'),
            ('future prediction', idx[test_end:future_end], future_pred[:, bi], 'red', 'lines'),
        ]
        traces = []
        for name, x, y, color, mode in raw_traces:
            traces.append(
                make_trace(
                    name, x, y, mode=mode, color=color, showlegend=(j == 0),
                    hovertemplate=(
                        f'{title}<br>'
                        'date=%{x|%Y-%m-%d}<br>'
    ...
    write_plotly_panels(
        output_path, 'Positron Flux Prediction', panels,
        columns=2, yaxis_title='Positron Flux',
    )
...
def write_error_html(output_path, idx, plot_bins, bin_labels, train_error, val_error, test_error):
    ...
    write_plotly_panels(
        output_path, 'Positron Flux Relative Error', panels,
...
for m in model_list:
    ...
    # =====================================================
    # 加载真实能量边界
    energy_edges = load_energy_edges()  # (28,) for 27 bins
    ...
    fig.supylabel('Positron Flux  [m$^{-2}$ s$^{-1}$ sr$^{-1}$ (GeV/n)$^{-1}$]', x=0.03, fontsize=14)
    fig.supxlabel('Year', y=0.03, fontsize=14)
    plt.savefig('./Figure/lstmdraw/positron_prediction_' + m + '.pdf', bbox_inches='tight')
    plt.close()
    write_prediction_html(
        './Figure/lstmdraw/positron_prediction_' + m + '.html',
    ...
    fig.supxlabel('Year', y=0.03, fontsize=14)
    plt.savefig('./Figure/lstmdraw/positron_error_' + m + '.pdf', bbox_inches='tight')
    plt.close()
    write_error_html(
        './Figure/lstmdraw/positron_error_' + m + '.html',

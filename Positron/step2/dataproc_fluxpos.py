...
# 1. 读取 ROOT 文件
# =====================================================
f = uproot.open("../../rawdata/flux/posflux.root")
h2 = f["hflux_2d;1"]

flux_2d = h2.values()   # (6000, 29)
err_2d  = h2.errors()   # (6000, 29)
...
# 能量轴 (变宽 bin)
y_edges = np.array(h2.axis(1).member('fXbins'))  # 30 edges, 29 bins

# =====================================================
# 3. 能量筛选: 跳过第一个和最后一个问题 bin
# =====================================================
# bin 0: [0.8, 1.0] GeV; last bin: [41.9, 45.1] GeV.
keep_bins = np.arange(1, y_nbins - 1)  # bins 1..27
flux_2d = flux_2d[:, keep_bins]
err_2d  = err_2d[:, keep_bins]
y_edges_kept = y_edges[keep_bins[0]:keep_bins[-1] + 2]  # edges for kept bins

n_days, n_bins = flux_2d.shape
print(
    f"After E cut ({y_edges_kept[0]:.1f}-{y_edges_kept[-1]:.1f} GeV): "
    f"({n_days}, {n_bins})"
)
...
np.save("Data/flux/positron_flux_allbin.npy", flux_filled)
np.save("Data/flux/positron_flux_abs_error_allbin.npy", abs_error)
np.save("Data/flux/positron_flux_rela_error_allbin.npy", rela_error)
np.save("Data/flux/positron_flux_ave_allbin.npy", flux_ave)
np.save("Data/flux/positron_energy_edges.npy", y_edges_kept)

print("\nSaved:")
print(f"  Data/flux/positron_flux_allbin.npy            {flux_filled.shape}")
print(f"  Data/flux/positron_flux_abs_error_allbin.npy  {abs_error.shape}")
print(f"  Data/flux/positron_flux_rela_error_allbin.npy {rela_error.shape}")
print(f"  Data/flux/positron_flux_ave_allbin.npy        {flux_ave.shape}")
print(f"  Data/flux/positron_energy_edges.npy           {y_edges_kept.shape}")
...
def write_flux_html(output_path, trace_mode='lines', with_error=False):
    ...
    write_plotly_panels(
        output_path, 'Positron Flux Overview', panels,
        columns=2,
        yaxis_title='Positron Flux [m^-2 s^-1 sr^-1 (GeV/n)^-1]',
    )
...
fig.supylabel('Positron Flux  [m$^{-2}$ s$^{-1}$ sr$^{-1}$ (GeV/n)$^{-1}$]', x=0.03, fontsize=13)
plt.savefig('Figure/flux/positron_flux_overview.pdf', bbox_inches='tight')
plt.close()
write_flux_html('Figure/flux/positron_flux_overview.html', trace_mode='lines', with_error=False)
...
fig.supxlabel('Year', y=0.03, fontsize=13)
fig.supylabel('Positron Flux  [m$^{-2}$ s$^{-1}$ sr$^{-1}$ (GeV/n)$^{-1}$]', x=0.03, fontsize=13)
plt.savefig('Figure/flux/positron_flux_overview_points.pdf', bbox_inches='tight')
plt.close()
write_flux_html('Figure/flux/positron_flux_overview_points.html', trace_mode='markers', with_error=False)
...
fig.supxlabel('Year', y=0.03, fontsize=13)
fig.supylabel('Positron Flux  [m$^{-2}$ s$^{-1}$ sr$^{-1}$ (GeV/n)$^{-1}$]', x=0.03, fontsize=13)
plt.savefig('Figure/flux/positron_flux_overview_with_error.pdf', bbox_inches='tight')
plt.close()
write_flux_html('Figure/flux/positron_flux_overview_with_error.html', trace_mode='markers', with_error=True)

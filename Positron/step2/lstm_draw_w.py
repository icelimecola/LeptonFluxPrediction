...
FLUX_PATH = os.environ.get('POSITRON_FLUX_PATH', 'Data/flux/positron_flux_allbin.npy')
ERROR_PATH = os.environ.get('POSITRON_ERROR_PATH', 'Data/flux/positron_flux_abs_error_allbin.npy')
...
# 只组合观测数据
Series = np.concatenate([sun_daily[sun_offset:sun_offset+number], positron_daily, positron_error_daily], axis=1)
...
def write_prediction_html(output_path, idx, plot_bins, bin_labels,
    ...
    write_plotly_panels(
        output_path, 'Weighted Positron Flux Prediction', panels,
...
def write_error_html(output_path, idx, plot_bins, bin_labels, train_error, val_error, test_error):
    ...
    write_plotly_panels(
        output_path, 'Weighted Positron Flux Relative Error', panels,

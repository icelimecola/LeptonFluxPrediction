...
flux_path = config.get('flux_path', 'Data/flux/positron_flux_allbin.npy')
error_path = config.get('error_path', 'Data/flux/positron_flux_abs_error_allbin.npy')
...
# 自动计算 sun offset: sun 从 2010-05-20 起, 补零后正电子从 2010-06-11 起
...
# sun-only input features. Flux is used only as the prediction target, and
# error is used only to build sample weights.
X_all = sun_daily[sun_offset:sun_offset+number, 0:5]
...
# 后边的训练数据需要加上上一年的太阳数据
X_train = X_all[0:train_end, :]
y_train = positron_daily[0:train_end, :]
err_train = positron_error_daily[0:train_end, :]
X_val = X_all[train_end-look_back:val_end, :]
y_val = positron_daily[train_end-look_back:val_end, :]
err_val = positron_error_daily[train_end-look_back:val_end, :]
X_test = X_all[val_end-look_back:test_end, :]
y_test = positron_daily[val_end-look_back:test_end, :]
err_test = positron_error_daily[val_end-look_back:test_end, :]
...
err_train = scaler_err.fit_transform(err_train)
err_val = scaler_err.transform(err_val)
err_test = scaler_err.transform(err_test)
...
# → (样本数, look_back, 5) , (样本数,bins), (样本数,bins)
...
plt.savefig('./Figure/lstmtrainw/loss_sunOnlyWeighted_'
        +str(epoch_begin)+'-'
        +str(epoch_end)+'epoch_'
        +str(neurons)+'neurons_'
        +str(l1)+'l1_'

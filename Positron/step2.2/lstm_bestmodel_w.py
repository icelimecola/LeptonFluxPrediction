...
def parse_model(path):
    ...
    row['l1'] = 0.0 if row['l1'] is None else float(row['l1'])
    row['l2'] = float(row['l2'])
...
def main():
    ...
    rows = []
    for path in MODEL_DIR.glob('sunOnlyWeighted_*.keras'):
        row = parse_model(path)
        if row is not None:
            rows.append(row)

    if not rows:
        raise FileNotFoundError(f'No sun-only weighted keras model found in {MODEL_DIR}')
    ...
    fields = [
        ...
        'learning_rate', 'neurons', 'l1', 'l2', 'dropout', 'batch_size',
        'train_num', 'val_num',
    ]
    ...
    print('best sun-only weighted model:', best['model'])
